...
var Engine = (function(global) {
    ...
    function main() {
        ...
        /* 设置我们的 lastTime 变量，它会被用来决定 main 函数下次被调用的时间。 */
        lastTime = now;
    ...
    /*
     * 这个函数在每一个时间间隙都会被调用，用来检测敌人对象和玩家的位置关系，
     * 从而确定他们是否发生碰撞
     */
    function checkCollisions() {
        for( let i=0; i<allEnemies.length; i++ ){
            let distance = Math.sqrt(Math.pow((allEnemies[i].x - player.x), 2) + Math.pow((allEnemies[i].y - player.y), 2) )
            if(distance <= 40){
                console.log( "撞到了" )
    ...
    function updateEntities(dt) {
        allEnemies.forEach(function(enemy) {
            enemy.update(dt);     // 更新每个敌人的数据
        });
        player.update();   // 更新玩家的数据
    }
    ...
    function renderEntities() {
        ...
        allEnemies.forEach(function(enemy) {
            enemy.render();     // 更新每个敌人的位置
        });

        player.render();    // 更新玩家的位置
    }
    ...
    Resources.onReady(init);    // 图片资源加载完毕 则调用 init 方法
